...
func main() {
	...
	logger.AppLogger.Info("Initializing Firebase authentication...")
	if err := api.InitializeAuth(); err != nil {
		logger.AppLogger.WithError(err).Error("Failed to initialize authentication - API will run without auth")
		println("[WARNING] Failed to initialize authentication:", err.Error())
	} else {
		logger.AppLogger.Info("Firebase authentication initialized successfully")
	}
